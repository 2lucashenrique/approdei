...
interface WeeklyRefuelChartProps {
  refuels: Refuel[];
  dateFilter?: DateFilterOptions;
  onWeekChange?: (start: Date, end: Date) => void;
}

const WeeklyRefuelChart: React.FC<WeeklyRefuelChartProps> = ({ refuels, dateFilter, onWeekChange }) => {
  ...
  const chartData = weekDays.map(day => {
    const dayRefuels = refuels.filter(refuel => 
      isSameDay(new Date(refuel.date + 'T12:00:00'), day)
    );
    
    const totalSpent = dayRefuels.reduce((sum, refuel) => sum + refuel.totalValue, 0);
    const totalLiters = dayRefuels.reduce((sum, refuel) => sum + refuel.liters, 0);
  ...
  const totalWeekSpent = chartData.reduce((sum, data) => sum + data.spent, 0);
  const totalWeekLiters = chartData.reduce((sum, data) => sum + data.liters, 0);
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle>Gastos com Abastecimento</CardTitle>
                ...
                <ChartTooltipContent
                  ...
                  labelFormatter={(label, payload) => {
                    if (payload && payload[0]) {
                      ...
                      return `${label} (${data.fullDate}) - ${data.liters.toFixed(2)} L`;
                    }
                    return label;
                  }}
                />
...
export default WeeklyRefuelChart;
